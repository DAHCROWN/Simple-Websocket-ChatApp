...
export default function ChatInterface({ session, onLeave }: ChatInterfaceProps) {
    ...
    <div className="flex flex-col h-screen bg-background">
      ...
      <div className="border-b border-border bg-card px-6 py-4">
        <div className="flex items-center justify-between max-w-4xl mx-auto">
          <div>
            <h1 className="text-2xl font-bold text-foreground">{session.room.name}</h1>
            <p className="text-sm text-muted-foreground">Joined as {session.username}</p>
          </div>
          <Button onClick={handleLeave} variant="outline" size="sm" className="gap-2 bg-transparent">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          {messages.map((message) => (
            <div
              key={message.id}
              className={`flex ${message.username === session.username ? "justify-end" : "justify-start"}`}
            >
              <div
...
async function connectToRoom(roomId: string, username: string): Promise<WebSocket> {
  return new Promise((resolve, reject) => {
    try {
      const protocol = window.location.protocol === "https:" ? "wss:" : "ws:"
      const wsUrl = `${protocol}//${window.location.host}/api/ws?roomId=${roomId}&username=${encodeURIComponent(username)}`
